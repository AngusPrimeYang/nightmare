...
ENightmareEnemyLocomotionType UNightmareEnemyRoller::RollLocomotionType()
{
	return RollLocomotionTypeWithStream(RandomStream);
}

ENightmareEnemyLocomotionType UNightmareEnemyRoller::RollLocomotionTypeWithStream(FRandomStream& Stream) const
{
	const float Chance = FMath::Clamp(HoverLocomotionChance, 0.0f, 1.0f);
	return (Stream.FRand() < Chance)
		? ENightmareEnemyLocomotionType::Hover
		: ENightmareEnemyLocomotionType::Walk;
}

float UNightmareEnemyRoller::RollHealth()
{
	return RollHealthWithStream(RandomStream);
}

float UNightmareEnemyRoller::RollHealthWithStream(FRandomStream& Stream) const
{
	const float Lo = FMath::Min(HealthMin, HealthMax);
	const float Hi = FMath::Max(HealthMin, HealthMax);
	if (FMath::IsNearlyEqual(Lo, Hi))
	{
		return FMath::Max(1.0f, Lo);
	}
	return FMath::Max(1.0f, Stream.FRandRange(Lo, Hi));
}
