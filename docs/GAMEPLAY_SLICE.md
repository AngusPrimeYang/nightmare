...
| # | 狀態 | 項目 | 備註 |
|---|------|------|------|
| G1 | `[ ]` | **美式加油站灰盒** | 世界座標中心；帶商店；低面數、不貼材質；方塊盡量多拼細節。**方案與元件清單 → [§7.1.1](#711-g1-加油站灰盒建議結構)** |
| G2 | `[ ]` | **角色灰盒** | 靜態方塊人即可（不做走路擺肢） |
| G3 | `[ ]` | **物品灰盒** | 持續緩慢旋轉的大方塊；建議父類／掛在 `ANightmarePickupActor`（或 `BP_Pickup`）；不貼材質 |
...
| # | 狀態 | 項目 | 備註 |
|---|------|------|------|
| P1 | `[ ]` | **刷物呈現＋生成時 roll** | 隨時間隨機出現；出現時懸浮低空、玩家移動可觸碰的位置。**生成當下**才決定 ±體力與數值大小。規則層已有 Scheduler／ItemDef／Pickup 殼；缺世界刷出、選位、roll、呈現與觸碰橋接。 |
| P2 | `[ ]` | **物品兩種互動** | (A) **碰到直接反應**（觸發當下改體力）；(B) **持有後使用**（現有：撿→持有→使用）。**契約：**(1) 互動方式 (A)/(B) **互斥**，**生成當下**隨機決定其一；(2) 每個隨機生成物品**只帶一種效果**（本條為體力 ±；與 **P8** 互斥，不可同體疊加）。需擴契約／Spec。 |
...
| P8 | `[ ]` | **物品：短暫加速／減速** | 新互動效果類型：短暫改變玩家移動速度（加速或減速）。互動方式與 **P2** 對齊且同樣互斥：(A) **碰到直接反應** 或 (B) **持有後使用**，**生成當下**隨機決定其一。`SpeedMultiplier`、`Duration`（及合理 Min/Max）參數化；**生成當下**隨機賦予該個體。**契約：與 P2 體力效果互斥——每個隨機生成物品只會有一種效果**（體力 **或** 加速／減速，不可兩者並存）。需 Spec（可注入時間／假隨機）。 |
